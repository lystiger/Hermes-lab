...
- **Total Backend Pytest Tests**: **293 tests** (291 passed, 2 skipped, 0 failed).
...
- **Phase 9.1 & Durability Hardening Suite (`tests/test_phase9_1_durability.py`)**: 13 tests verifying synchronous persist precedence, fail-closed task and job completion errors, unswallowed exceptions in run loops, valid ledger prefixes, gap rejection, in-memory state rollback on persistence failure, synchronous/async cancel coordination, pre-seeded production task persistence, downstream task cancellation retries without duplicate terminal facts, and worker exception propagation.
