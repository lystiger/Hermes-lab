...
- **Total Backend Pytest Tests**: **292 tests** (290 passed, 2 skipped, 0 failed).
...
- **Phase 9.1 & Durability Hardening Suite (`tests/test_phase9_1_durability.py`)**: 12 tests verifying synchronous persist precedence, fail-closed task and job completion errors, unswallowed exceptions in run loops, valid ledger prefixes, gap rejection, in-memory state rollback on persistence failure, synchronous/async cancel coordination, pre-seeded production task persistence, and failure-injection retries for job-creation, partial seed tasks, and cancellation.
