            ...
            try:
                async with self.session_factory() as session:
                    async with session.begin():
                        # 1. Check idempotency: check if event_id already exists
                        ...
                        # 2. Acquire transaction-level advisory lock on hashtext(job_id) if Postgres
                        # This serializes concurrent appends for the same job within the database transaction
                        try:
                            ...
                            # Non-postgres dialects in test environments may skip advisory lock
                            pass
                ...
                if "uq_runtime_events_job_sequence" in err_msg or "unique constraint" in err_msg:
